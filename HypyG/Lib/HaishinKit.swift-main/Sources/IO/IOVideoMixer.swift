...
public protocol IOVideoMixerGpuPixelDelegate: AnyObject {
    func processImageBuffer(sampleBuffer: CMSampleBuffer)
}

public protocol IOVideoMixerDelegate: AnyObject {
...
public class IOVideoMixer<T: IOVideoMixerDelegate> {
    weak var delegate: T?
    ...
    public var gpuDeleage: IOVideoMixerGpuPixelDelegate?

    deinit {
        print("IOVideoMixer deinit")
    }
    func append(_ track: UInt8, sampleBuffer: CMSampleBuffer) {
        ...
        case .passthrough:
            if settings.mainTrack == track {
                if let gpuDeleage = self.gpuDeleage {
                    gpuDeleage.processImageBuffer(sampleBuffer: sampleBuffer)
                } else {
    ...
    func registerGpuPixelDelegate(_ delegate: IOVideoMixerGpuPixelDelegate?) {
        self.gpuDeleage = delegate
    }
...
extension IOVideoMixer: ScreenObserver {
    public func screen(_ screen: Screen, didOutput sampleBuffer: CMSampleBuffer) {
