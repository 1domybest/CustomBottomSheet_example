...
final class IOVideoUnit: IOUnit {
    ...
    func registerGpuPixelDelegate(_ delegate: IOVideoMixerGpuPixelDelegate?) {
        videoMixer.registerGpuPixelDelegate(delegate)
    }
    
    func processVideo(_ sampleBuffer: CMSampleBuffer) {
        videoMixer.outputSampleBuffer(sampleBuffer)
    }
    ...
    func attachCamera(_ device: AVCaptureDevice?, track: UInt8, configuration: IOVideoCaptureConfigurationBlock?) throws {        
        ...
        if let capture = captures[track] {
            if let oldDevice = capture.device {
                guard oldDevice != device else {
                    return
                }
            }
        }
